...
export default function PublicationsList({ config, publications, embedded = false }: PublicationsListProps) {
    ...
    const groupedPublications = useMemo(() => {
        const journalPubs = publications.filter(p => p.journal);
        const conferencePubs = publications.filter(p => p.conference);
        const preprintPubs = publications.filter(p => !p.journal && !p.conference);
        return { journal: journalPubs, conference: conferencePubs, preprint: preprintPubs };
    }, [publications]);
            ...
            >
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-grow">
                        ...
                        <p className="text-sm font-medium text-neutral-800 dark:text-neutral-600 mb-3 italic">
    ...
    return (
        ...
        >
            ...
            {/* Group by Journal and Conference */}
            <div className="space-y-8">
                {groupedPublications.conference.length > 0 && (
                    <div>
                        <h2 className="text-2xl font-serif font-bold text-primary mb-4">Conference Papers</h2>
                        <div className="space-y-4">
                            {groupedPublications.conference.map((pub, idx) => renderPublication(pub, idx))}
                        </div>
                    </div>
                )}

                {groupedPublications.journal.length > 0 && (
                    <div>
                        <h2 className="text-2xl font-serif font-bold text-primary mb-4">Journal Papers</h2>
                        <div className="space-y-4">
                            {groupedPublications.journal.map((pub, idx) => renderPublication(pub, idx + groupedPublications.conference.length))}
                        </div>
                    </div>
                )}

                {groupedPublications.preprint.length > 0 && (
                    <div>
                        <h2 className="text-2xl font-serif font-bold text-primary mb-4">Preprints</h2>
                        <div className="space-y-4">
                            {groupedPublications.preprint.map((pub, idx) => renderPublication(pub, idx + groupedPublications.conference.length + groupedPublications.journal.length))}
                        </div>
                    </div>
                )}
            </div>
